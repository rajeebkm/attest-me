...
function AttestationsView() {
  ...
  const contract = useMemo(
    () => new Contract(AttestationsAbi, SAS, provider),
    [provider]
  );
  ...
  const fetchAttestationsDetails = async () => {
    ...
    try {
      ...
      if (result && result.data) {
        ...
        if (parsedData.uid !== "") {
          const attestationDetails: AttestationData = {
            ...
            revoked: parsedData.isRevoked ? "Yes" : "No",
            revocable: parsedData.revocable.toString(),

            attestedData: parsedData.data.toString(),
          };
          setAttestationData(attestationDetails);
          console.log("attestationDetails", attestationDetails.schemaUID);
        } else {
          toast.error("Invalid Attestation UID");
  ...
  const { writeAsync } = useContractWrite({
    calls: useMemo(() => {
      if (!attestationData?.schemaUID) return null;

      return contract.populateTransaction["revoke"]({
    ...
    }, [contract, attestationUID, attestationData?.schemaUID]),
  });
  ...
  const handleRevoke = async () => {
    ...
    try {
      ...
      toast.dismiss();
      toast.error("Failed to revoke attestation. Please try again.");
        ...
        <div className="flex flex-col md:flex-row items-center gap-10">
          ...
          <div className="flex flex-1 flex-col sm:flex-row items-center gap-1 w-full">
            ...
            <button
              ...
              className={`py-2 px-3 rounded-lg font-semibold transition-all duration-300 ease-in-out transform shadow-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-opacity-50 ${
                loading || !attestationUID
        ...
        {!loading && attestationData && isConnected && (
          <>
            ...
            <div className="mb-4 relative flex flex-col w-full max-w-6xl rounded-lg mt-6 p-4 px-10 py-4 transition-colors dark:text-gray-700 dark:hover:text-gray-700 shadow-2xl bg-opacity-20 dark:bg-opacity-20">
              <h2 className="text-xl font-semibold text-gray-800 dark:text-gray-600">
                Attested Schema Data:
              </h2>
              <div
                className={`text-gray-600 dark:text-gray-500 overflow-auto max-h-80 ${
                  isRawDataExpanded ? "max-h-full" : ""
                }`}
                style={{
                  whiteSpace: "pre-wrap",
                  width: "100%",
            ...
            <div className="justify-center text-center mt-10">
              <button
                onClick={openModal}
                className="bg-blue-600 py-3 px-6 rounded text-white hover:bg-blue-700 disabled:bg-gray-100 disabled:text-gray-600 disabled:cursor-not-allowed transition duration-300 ease-in-out transform hover:scale-105 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-opacity-50"
                disabled={
                  !isConnected ||
                  !account ||
                  account.address !== attestationData?.from ||
                  attestationData?.revocable !== "true" ||
                  attestationData?.revoked !== "No"
                }
              >
                Revoke Attestation
              </button>
